...
parser.add_argument('-b', '--bed', help='BED file with ancestral alleles in 4th column and conservation'
                    ...
                    required=True)
parser.add_argument('-f', '--fd', help='Sites info file.', type=str, required=False)
parser.add_argument('-l', '--lines', help='Total lines in file (used for counting progress.', type=str, required=True)
...
# Here, we read the BED file and save it.
def read_bed_file(bed_file):
    ...
    with open(bed_file, 'r') as bed:
        for line in bed:
            ...
            aa_bed_data[(chrom, int(start))] = allele
            cs_bed_data[(chrom, int(start))] = cs
    return aa_bed_data, cs_bed_data

# Here we loop over variants and look for AA and CS for given variant.
def update_vcf_with_bed_info(vcf_file, aa_bed_data, cs_bed_data, output_file):
    with (open(vcf_file, 'r') as vcf, open(output_file, 'w') as out):
        ...
        for line in vcf:
            ...
            # Adding lines to VCF header about added fields.
            if line.startswith('#'):
            ...
            if args.fd is not None:
                with open(args.fd, 'a') as x:
                    print("Reason: ", reason, file=x, end="\n")
...
update_vcf_with_bed_info(args.vcf, *AA_data, args.out)
